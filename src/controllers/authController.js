...
async function login(req, res) {
  ...
  try {
    ...
    // Discord-only accounts have no password — can't log in this way
    if (!user.password) {
      return res.status(400).json({ 
        message: "This account was created via Discord. Please log in through Discord or set a password in settings." 
      });
    }
